...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $sql = 'SELECT id, email, password, ime, prezime, is_active FROM users WHERE email = :email';
  $stmt = $db->prepare($sql);
        ...
        <div class="col-md-6 mx-auto">
          <div class="card">
            <div class="card-header">
              <h3 class="mb-0">Prijava u sustav</h3>
            </div>
            <div class="card-body">
              <form class="form" role="form" method="POST" action="login.php" autocomplete="off">
                ...
                <button type="submit" class="btn btn-info float-right">Prijava</button>
              </form>
            </div>
          </div>
        </div>
